...
export default function LoginPage() {
  ...
  return (
    <>
      ...
      <div className="relative min-h-screen flex items-center justify-center p-6 overflow-hidden bg-white text-black">
        <div className="w-full max-w-lg rounded-3xl shadow-[0_15px_60px_rgba(0,0,0,0.08)] border border-black/10">
          <div className="rounded-3xl bg-white">
            <div className="p-8">
              <div className="text-center">
                <h1 className="text-3xl font-semibold tracking-tight">Welcome back</h1>
                <p className="mt-1 text-sm text-black/70">Sign in to continue</p>
              </div>

              <div className="mt-5 flex items-center justify-center">
                <div className="inline-flex rounded-full bg-white border border-black/10 shadow-sm overflow-hidden">
                  <button onClick={() => setMode('signin')} className={`px-4 py-2 text-sm font-medium transition ${mode==='signin' ? 'btn-blue text-white' : 'hover:bg-black/5'}`}>Sign in</button>
                  <button onClick={() => setMode('signup')} className={`px-4 py-2 text-sm font-medium transition ${mode==='signup' ? 'btn-blue text-white' : 'hover:bg-black/5'}`}>Create</button>
                  <button onClick={() => setMode('forgot')} className={`px-4 py-2 text-sm font-medium transition ${mode==='forgot' ? 'btn-blue text-white' : 'hover:bg-black/5'}`}>Forgot</button>
                </div>
              </div>
                ...
                <form onSubmit={handleEmailPassword} className="mt-6 space-y-5">
                  <div>
                    ...
                    <input type="email" placeholder="you@example.com" value={email} onChange={(e) => setEmail(e.target.value)}
                           className="w-full rounded-xl bg-white border border-black/10 px-4 py-3 placeholder-black/40 focus:outline-none focus:ring-2 focus:ring-[#0038b8]" />
                  </div>
                  <div>
                    <label className="block text-sm mb-1">Password</label>
                    <div className="flex rounded-xl overflow-hidden border border-black/10 bg-white">
                      <input type={showPassword ? 'text' : 'password'} placeholder="••••••••" value={password} onChange={(e) => setPassword(e.target.value)}
                             className="w-full px-4 py-3 placeholder-black/40 focus:outline-none" />
                      <button type="button" onClick={() => setShowPassword((v) => !v)} className="px-4 text-sm font-medium hover:bg-black/5">{showPassword ? 'Hide' : 'Show'}</button>
                    </div>
                  </div>
                  <button type="submit" disabled={loading}
                          className={`w-full rounded-full py-3 font-semibold shadow transition transform ${loading ? 'opacity-70' : 'hover:shadow-lg hover:-translate-y-0.5'} btn-blue`}>
                    Sign in
                  </button>
                  <div className="grid grid-cols-2 gap-3">
                    <button type="button" onClick={handleMagicLink} disabled={loading}
                            className="w-full rounded-full py-2 font-medium btn-white">Magic link</button>
                    <button type="button" onClick={handleGoogle} disabled={loading}
                            className="w-full rounded-full py-2 font-medium border-2 border-black bg-white hover:bg-black/5 transition flex items-center justify-center gap-2">
